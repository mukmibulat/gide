...
| Перед `return`.                                                      | Одна |
| Перед `raise`.                                                       | Одна |
| После _docstring_, для отделения его от тела функции/метода/класса.  | Одна |
...
Примечание: в методе, который состоит из _docstring_ и одной строки тела - ставится одна пустая строка между ними.
```python
def get_code():
    """Возвращает код."""
